...
public class World : MonoBehaviour {
    ...
    void BuildChunkAt(Vector3 chunkPos) {
        ...
        if (!chunkDict.TryGetValue(name, out c)) {
            ...
            chunkDict.TryAdd(c.chunk.name, c);
        }
    }
    ...
    IEnumerator DrawChunks () {
        ...
        foreach(KeyValuePair<string, Chunk> c in chunkDict) {
            ...
            if (c.Value.chunk && Vector3.Distance(player.transform.position, c.Value.chunk.transform.position) > chunkSize * radius) {
                toRemove.Add(c.Key);
            }
        }
    ...
    private void Start() {
        ...
        Building(WhichChunk(player.transform.position), radius);
        Drawing();
